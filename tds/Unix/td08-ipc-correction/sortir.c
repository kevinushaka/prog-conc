...
int main() {
  ...
  */
  ...
  /* On ne fait que sortir : on consomme donc une place vide */
  ...
  while (1) {
    ...
    down(plein); // On sort, il y a une place pleine de moins
    up(vide); // On est sorti, il y a une place vide de plus

    down(mutex); // On va modifier le nombre de place, il faut donc être en section critique
    *shmadr = (*shmadr)+1;
    printf("Nombre de place vide restante %d\n", *shmadr);
    i = *shmadr; // On fait une copie de la valeur pour pouvoir l'utiliser en dehors d'une section critique
    up(mutex);
